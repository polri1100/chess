...
class Pawn(Piece):
    ...
    def movements(self):
        ...
        y = self.rect.y
        if self.color_white:
            possible_moves = [(x,y-100)]
            if y == 600:
                possible_moves.append((x,y-200))
            print(x//size-1, y//size-1)
            print(board.array[x//size-1][y//size-1])
            if board.array[x//size-1][y//size-1] != None:
                possible_moves.append((x-100,y-100))
        else:
            possible_moves = [(x,y+100)]
            if y == 100:
                possible_moves.append((x,y+200))
...
def paint_possible_moves(possible_moves):
    for move in possible_moves:
        possible_mov = pygame.Surface((size,size), pygame.SRCALPHA)
        possible_mov.fill((255,255,255,128))
        win.blit(possible_mov, move)
        pygame.display.update() 
...
def main():
    ...
    while run:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and not make_move:
                x, y = event.pos
                ...
                if len(clicked_sprites) != 0 and clicked_sprites[0].color_white == turn_white:
                    ...
                    select_piece(y, x)
                    paint_possible_moves(piece.movements())
                    oldx, oldy = x//size, y//size
                    make_move = True

            elif event.type == pygame.MOUSEBUTTONDOWN and make_move:
